      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {journals.map((journal) => (
          ...
          >
            ...
            <CheckCircle2 className={`h-5 w-5 transition-opacity duration-500 ${journal.selected ? "opacity-100" : "group-hover/journal:opacity-20 opacity-0"}`} />
          </button>
        ))}
      </div>

      <Button variant="outline" className="w-full font-semibold text-emerald-500 hover:text-emerald-400 bg-gray-800/10 border-gray-700/30 hover:bg-gray-700/30">
        + Add New Journal
      </Button>
    </div>
  )
